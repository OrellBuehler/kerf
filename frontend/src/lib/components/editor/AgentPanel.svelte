<script lang="ts">
	...
	// Activity feed derived from each task's latest transition (newest first).
	type Activity = { time: string; icon: string; text: string; who: string };
	const ACTIVITY: Record<TaskStatus, { icon: string; who: string; verb: string }> = {
		queued: { icon: 'list-plus', who: 'you', verb: 'Queued' },
		working: { icon: 'hand', who: 'agent', verb: 'Agent claimed' },
		ready: { icon: 'git-pull-request-arrow', who: 'agent', verb: 'Agent staged' },
		done: { icon: 'check', who: 'you', verb: 'Applied' },
		failed: { icon: 'history', who: 'agent', verb: 'Agent failed' }
	};
	const activity = $derived<Activity[]>(
		[...agent.tasks]
			...
			.slice(0, 6)
			.map((t) => {
				const a = ACTIVITY[t.status];
				return { time: clock(t.updated_at), icon: a.icon, who: a.who, text: `${a.verb} “${t.prompt}”` };
			})
	);
		...
		<div>
			...
			<div style="display:flex;flex-direction:column;gap:1px">
				{#if activity.length === 0}
					<div style="font-size:12px;color:var(--text-muted);padding:6px 4px">No activity yet.</div>
				{:else}
					{#each activity as a, i (i)}
						<div style="display:flex;gap:9px;align-items:flex-start;padding:6px 4px">
							<span
								style="font-family:var(--font-mono);font-size:10px;color:var(--text-disabled);width:36px;flex:none;padding-top:2px;text-align:right"
								>{a.time}</span
							>
							<Icon n={a.icon} s={13} color={tint[a.who]} style="margin-top:1px" />
							<span style="font-size:12px;line-height:1.45;color:var(--text-secondary)">{a.text}</span>
						</div>
					{/each}
				{/if}
			</div>
