...
LLM claims tasks over MCP. The queue is `agent` state (`src/lib/agent.svelte.ts`, the third
runes singleton) backed by the `tasks` table over Tauri/MCP: the add-task box and preset chips
`agent.add(...)` real tasks, `ready` tasks show Apply/Dismiss (`resolve_task`/`remove_task`),
and the activity log is derived from each task's latest state transition. Two preset chips
(`Remove silences` / `Assemble rough cut`) also run the matching local op and resolve their task;
the rest just enqueue for the agent. In the browser there is no agent, so queued tasks correctly
just wait. `data.ts` keeps only the `STATUS_MAP`/`PRESETS` presentation bits and the fallback bin.
